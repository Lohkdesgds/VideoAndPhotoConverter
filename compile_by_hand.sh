...
cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$OUTPUT -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=$OUTPUT -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$OUTPUT $ARGUMENT -S . -B build --debug-output 2>&1 | tee runner.txt
cmake --build build -v 2>&1 | tee runner.txt -a;

...
void MAGICK::call(const std::vector<std::string>& cmds) const
{
	const auto& path = m_pathing.get_magick_exe();
...
void MAGICK::call(const std::vector<std::string>& cmds, const std::function<void(const std::string&)>& out) const
{
	const auto& path = m_pathing.get_magick_exe();

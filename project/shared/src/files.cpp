...
{
#ifdef _WIN32
	if (m_path.find(":" SLASH) == std::string::npos) throw std::invalid_argument("Path must be canonical (e.g. not relative)");
#else
	if (m_path.find(SLASH) != 0) throw std::invalid_argument("Path must be canonical (e.g. not relative)");
#endif
}
...
void VideoFile::convert() const
{
	...
	m_ffmpeg.call(params, [&](const std::string& l) {
